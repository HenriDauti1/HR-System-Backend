...
public class PayrollService {
    ...
    public List<PayrollResponse> getAllPayrolls() {
        List<Payroll> payrolls = payrollRepository.findAllByEmployee_IsActiveTrue();
        return payrolls.stream().map(this::convertToResponse).collect(Collectors.toList());
    }
    ...
    public PayrollResponse updatePayroll(UUID id, UpdatePayrollRequest request) {
        Payroll payroll = payrollRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Payroll not found with id: " + id));
